 ...
 */
struct Message {
    // from AMBASSADOR.h messageType from the event loop
    // enum messageType
    // {
    //     INPUT_REQ,
    //     CONFIG_REQ,
...
class ActionEventLoop {
public:
    virtual void executeEventLoopMsg(Server& server, ambassador::Ambassador& loopAmbassador, ambassador::Response res) = 0;
};

class ActionClient {
public:
    virtual MessageResult executeClientMsg(Server& server, const Message& message) = 0;
};
